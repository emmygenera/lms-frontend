  ...
  const handleSubmit = (e) => {
    ...
    User.login(user.email, user.password)
      ...
      .catch((err) => {
        ...
        else toast.error("Oops! Error occurd: " + err?.response?.statusText);
        // err.response.status;
      });
  };
    ...
    <Row style={{ height: "100vh" }}>
      <Col sm={12} md={6} className="leftdiv" style={{ position: "relative", display: "flex", justifyContent: "center", flexDirection: "column" }}>
        <div className="ms-1 ps-1 ms-md-5 ps-md-5 mt-5">
          ...
          <button type="button" className="btn btn-sm btn-outline-secondary" id="leftdivbtn" onClick={() => window.location.replace(baseUrl())}>
            SHOP NOW
          </button>
        </div>
