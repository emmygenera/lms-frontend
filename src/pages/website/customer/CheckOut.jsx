...
import { baseUrl, CountryData, DateTime, EmjsF, objectOnly, object_entries, Post, RandomString, toCapitalize } from "../../../applocal";
import LoadingAnim from "../../../components/LoadingAnim";
import { setCustomer } from "../../../redux/actions/auth";
import { CourseLists } from "../CreateUserOrder";
// import Body from "../Body";
import PaymentAPI from "../../../services/PaymentAPI";
import { clearAddonsPurchase, clearCartItem, setPayment } from "../../../redux/actions";
...
// }
const orderCurrency = "jod";

export function totalPrice(cartsData_ = [], addonsData_ = []) {
  const tt = (key = orderCurrency, add_addonsTotal = true) => {
    const courseTotal = cartsData_
      .map((itm) => {
    ...
    const addonsTotal = add_addonsTotal
      ? addonsData_
          .map((itm) => {
            const { price } = itm["1"];
            return price[key];
          ...
          .reduce((l, r) => Number(l) + Number(r), 0)
      : 0;
    return courseTotal + addonsTotal;
  };
  return tt;
}
...
export default function CheckOut() {
  ...
  const { carts, addons_purchase, payment } = useSelector((s) => s.globals);
  ...
  const paymentData = object_entries(payment);

  const orderLen = cartsData.length > 0;
  const addonsLen = addonsData.length > 0;
  const paymentMade = paymentData.length > 0;
  ...
  const courseTotalPrice = totalPrice(cartsData, addonsData);

  function processOrder({ card, orderId: orderID }) {
    setOrderProcessing(true);
    if (addonsLen && !orderLen) return finalPurchaseProccess();
    ...
    const data = {
      ...
      total: courseTotalPrice(orderCurrency, false),
      userId: user?.id,
    ...
    orderService
      ...
      .then(({ data }) => {
        ...
        if (!addonsLen) window.location.href = baseUrl("#/myCourses");
        else finalPurchaseProccess();
      })
      .catch(() => {
        toast.error("Error occurred while processing course purchase... Please try again!");
      })
      .finally(() => {
        if (!addonsLen) setOrderProcessing(false);
      });
  }
  function processAddonPurchase() {
    const addonsMapData = addonsData.map((itm) => {
      const addonItem = itm["1"];
      return {
        userId: user?.id,
        addonId: addonItem?._id,
        purchaseId: RandomString(),
        paymentType: "Credit Card",
        amount: addonItem?.price[orderCurrency],
        currency: orderCurrency,
      };
    });
    return addonsMapData.map((data) => addonsAPI.purchaseAdd(data));
  }

  function finalPurchaseProccess() {
    Promise.allSettled(processAddonPurchase())
      .then(() => {
        dispatch(clearAddonsPurchase());
        window.location.href = baseUrl("#/");
      })
      .catch(() => {
        toast.error("Error occurred while processing final addons purchase... Please contact for report!");
      })
      .finally(() => {
  ...
  const addPayment = (v) => {
    if (paymentMade) {
      return processOrder({ card: v, orderId: payment?.orderId });
    }
        ...
        <div className="col-5">
          {orderLen && (
            <div className="outline-shadow bg-white mt-5">
              <h3 className="fw-bold p-3  fz-3">Courses Purchase Information</h3>
          ...
          {addonsLen && (
            <div className="outline-shadow bg-white mt-5">
              <h3 className="fw-bold p-3  fz-3">Addons Purchase Information</h3>

              {addonsData.map((itm) => {
                const addonItem = itm["1"];
                const { usd = "", jod = "" } = addonItem?.price;

                return <CourseLists title={addonItem.title} date={"Expiry Date: " + DateTime(addonItem.expire_at).stringFormat()} amount={`$${usd} - ${jod} JOD`} />;
              })}
            </div>
          )}
          <div className="outline-shadow bg-white .mt-5">
