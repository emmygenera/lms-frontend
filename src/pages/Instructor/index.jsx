...
const Instructor = (props) => {
  ...
  const getData = () => {
    ...
    InstructorService.getPaginated(page, pageSize, search || "")
      .then(({ data: { data: instructors, total = 0 } }) => {
        ...
        setDate(instructors.map((category, index) => ({ ...category, status: category?.status == 1 ? "active" : "deactive", action: actions(category), id: index + 1, index: index + 1, join_date: moment(category.createdAt).format("L") })));
  ...
  const updateCat = (data) => props.history.push(`/newInstructor?data=${data?._id}`);
  ...
  // Testing
  const activeAll = () => {
    const rows = selectedRowKeys.map((id) => data.find(({ _id }) => _id == id)).map((item) => ({ id: item._id, status: 1 }));
    setLoading(true);
    Promise.all(rows.map(async (instructor) => await InstructorService.updateOne(instructor, instructor.id))).then(() => {
